...
function vnit_lang(){
    $CI = get_instance();
    $lang = $CI->session->userdata('lang_admin');
    if($lang == 'vietnamese'){
        return 'vi';
...
function get_discount($str){
    ...
    $gift1 = substr($pass, 0, 4);
    $gift2 = substr($pass, 4, 4);
    $gift3 = substr($pass, -4);
    return $gift = strtoupper($str.'-'.$gift1.'-'.$gift2.'-'.$gift3);               
}
...
function vnit_barcode($code,$number, $length=8){
...
function build_fea($feature_id,$type,$variant_id=0,$value=''){
    $list = get_var_fea($feature_id);
    if($type == "C"){
        $html = "";
        foreach($list as $val):
            $html .= '<input type="checkbox" value="'.$val->variant_id.'" name="variant_'.$feature_id.'">'.$val->variant;
        endforeach;
            $html .= '<input type="hidden" name="ar_var[]" value="'.$feature_id.'">';
            $html .= '<input type="hidden" name="variant_type_'.$feature_id.'" value="C">';
        return $html;
        
    }else if($type == "S"){
        $html ="<select style=\"width:200px;margin-right:10px;\" id=\"feature_$feature_id\" name=\"variant_$feature_id\" onchange=\"change_feature($feature_id)\">";
            $html .="<option value=\"\">-None-</option>";
            foreach($list as $val):
            $select = ($variant_id ==$val->variant_id )?'selected="selected"':'';
            $html .='<option value="'.$val->variant_id.'" '.$select.'>'.$val->variant.'</option>';
            endforeach;
            $html .="<option value=\"select_disable\">-Enter other</option>";
        $html .="</select>";
        
        $html .='<input type="text" name="new_variant_'.$feature_id.'" style="display:none;width: 200px" value="" id="input_'.$feature_id.'">';
        $html .= '<input type="hidden" name="ar_var[]" value="'.$feature_id.'">';
        $html .= '<input type="hidden" name="variant_type_'.$feature_id.'" value="S">';
        return $html;
    }else{
        $html ='<input type="text" style="width: 200px" value="'.$value.'" name="variant_'.$feature_id.'">';
        $html .= '<input type="hidden" name="ar_var[]" value="'.$feature_id.'">';
        $html .= '<input type="hidden" name="variant_type_'.$feature_id.'" value="T">';
        return $html;
    }
}

function get_var_fea($feature_id){
    $CI = get_instance();
    $CI->db->select('shop_feature_variants.*, shop_feature_variant_descriptions.*');
    $CI->db->join('shop_feature_variants','shop_feature_variants.variant_id = shop_feature_variant_descriptions.variant_id');
    $CI->db->where('shop_feature_variants.feature_id',$feature_id);
    $CI->db->order_by('shop_feature_variant_descriptions.variant','asc');

    return $CI->db->get('shop_feature_variant_descriptions')->result();
}

function vnit_editor($value,$name, $by,$readmore = TRUE){
    
    $data = '<textarea class="editor" name="'.$name.'" id="'.$by.'">'.$value.'</textarea>';
    $data .='
    ...
    ';
    if($readmore == TRUE){
        $data .='<br /><input type="button" name="insert_text" value="Đọc thêm" onclick="insertReadmore(\''.$by.'\');">';
    }
    return $data;
    
}

function vnit_editor_basic($value,$name, $by){
...
function vnit_change_string($value){
        #---------------------------------a^
