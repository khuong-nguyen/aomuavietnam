...
/*----------------------------------------------------------+
 | Ham tao ra 1 array bằng đệ qui    = buildArray           |
 +----------------------------------------------------------*/	
function buildArray($sourceArr, $parents = 0){    
	$resultArr = array();
	recursive($sourceArr,$parents,1, $resultArr);
	return $resultArr;
}
...
/*----------------------------------------------------------+
 | Ham tao  nhung id con cua id hien hanh                   |
 +----------------------------------------------------------*/
...
/*----------------------------------------------------------+
 | Ham tao  nhung id cha cua id hien hanh                   |
 +----------------------------------------------------------*/
function getParentsIdArray($sourceArr, $id) {
	$arrParents [] = $id;
...
/*----------------------------------------------------------+
 | Ham tao tim id cha cua id hien hanh                      |
 +----------------------------------------------------------*/
...
/*----------------------------------------------------------+
 | Ham tao menu vertical = dequi 'recursiveMenu'            |
 +----------------------------------------------------------*/
	...
	if (count ( $sourceArr ) > 0) {
		...
		foreach ( $sourceArr as $key => $value ) {
			if ($value ['parents'] == $parents) {
				...
				if ($value ['parents'] == 0) {
					$link = site_url ( 'thong-tin/' . stringchange ( $value ["name"] ) . '-' . $value ["id"] );
					$newMenu .= '<li><a id="' . $liMenu . '" href="' . $link . '">' . $value ['name'] . '</a>|';
				} else {
					$link = site_url ( 'thong-tin/' . stringchange ( $value ["name"] ) . '-' . $value ["id"] );
...
/*----------------------------------------------------------+
 | Ham lay id từ  1 strURL 'getIdUrl'                       |
 +----------------------------------------------------------*/
function getIdUrl($strUrl) {
	$pos = strrpos ( $strUrl, "-" );
...
/*----------------------------------------------------------+
 | Ham kiem tra file co ton tai hay k?                      |
 +----------------------------------------------------------*/
function isHTMLCat($file) {		
	if(file_exists( $file ) ){
		return true ;
	}		
	return false ;
...
/*----------------------------------------------------------+
 | Ham tao menu 1 cap getMenuParent                         |
 +----------------------------------------------------------*/
function getMenuParent($sourceArr, $nameCat= '', $atribule= array('thong-tin','type')){
	$file = ROOT."cache/html/menu/".$atribule['type']."_".".html";
	$options = "";
	if(isHTMLCat($file)) {
		...
		if(count($sourceArr)>0){
			foreach ($sourceArr as $k=>$v){
				if($nameCat !=''){
					$link	  = site_url($atribule['thong-tin']."/".stringchange($nameCat)."/".stringchange($v["name"])."-".$v["id"]);
				}else{
					$link	  = site_url($atribule['thong-tin']."/".stringchange($v["name"])."-".$v["id"]);
				}
				$options .= "<li>";
				$options .= '<a href="'.$link.'">'.$v["name"].'</a>';
				$options .= "</li>";
			}
		}
		//write file - - - - - - - - - -
...
/*----------------------------------------------------------+
 | Ham tao menu 1 cap getMenuParent                         |
 +----------------------------------------------------------*/
function getMenuParentID($sourceArr, $id, $atribule =array('thong-tin')){
	$file = ROOT."cache/html/menu/".$id."_".".html";
	$options = "";
	if(isHTMLCat($file)) {
		$options = @file_get_contents($file) ;			
	}else{
...
/*----------------------------------------------------------+
 | Ham tao cache file khi truyen vao  mang getCacheTable   |
 +----------------------------------------------------------*/
function getCacheTable($sourceArr, $pathCache, $optionsArr){
	// kiem tra co ton tai thu muc chua
	if(!file_exists($pathCache)){
		@mkdir($pathCache,0777);
	}
	
	$file = ROOT . $pathCache ."/". $optionsArr["nameCache"] ."_".".html";
	$options = "";
	if(isHTMLCat($file)) {
		$options = @file_get_contents($file) ;			
	}else{
				...
				if(!isset($val["link"])){
					switch ($val["type"]){
						case 'Yahoo':{
							...
							$options .='<a href="ymsgr:sendim?'.$val["nick"].'">
							    <img border="0" src="http://opi.yahoo.com/online?u='.$val["nick"].'&amp;m=g&amp;t=2&amp;1=us"/></a>';
							$options .='</li>';
							break;
						}
						case 'Skype':{
							$options .='<li>';
							$options .='<span>'.$val["name"].'</span>'; 
							$options .='<a href="skype:'.$val["nick"].'?chat"><img border=0 src="http://mystatus.skype.com/smallclassic/'.$val["nick"].'"></a>';
							$options .='</li>';
							break;
						}
						default:{
					...
					if(isset($optionsArr['boxID'])){
						$options .='<a href="'."http://".$val["link"].'" target="_blank"><img src="'.$optionsArr["imgPath"].$val["image"].'"></a>';
						
					}else{
						$idImage = "";
						$options .='<li>';
						$options .='<a href="'."http://".$val["link"].'" target="_blank" '.$optionsArr["classStyle"].'><img src="'.$optionsArr["imgPath"].$val["image"].'"></a>';
						$options .='</li>';
					}	
				}
...
/*----------------------------------------------------------+
 | Ham tao cache file quang cao                         |
 +----------------------------------------------------------*/
function getCacheAds($sourceArr, $pathCache, $optionsArr){
	// kiem tra co ton tai thu muc chua
	if(!file_exists($pathCache)){
		@mkdir($pathCache,0777);
	}
	...
	if(isHTMLCat($file)) {
		...
		if(count($sourceArr)>0){
			foreach ($sourceArr as $key=>$val){
				if(substr($val["image"],-3,3)=='swf'){
					$options .='<li>';
					$options .= showIMG(base_url()."uploads/ads/full_images/".$val["image"],190,170);
					$options .='</li>';
				}else{
					$options .='<li>';
					$options .='<a href="'."http://".$val["link"].'" target="_blank"><img src="'.$optionsArr["imgPath"].$val["image"].'" width="'.$optionsArr["width"].'"></a>';
					$options .='</li>';
				}
