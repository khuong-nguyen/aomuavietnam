<?php
class ordercart extends CI_Controller{
    protected $_templates;
    function __construct(){
        parent::__construct();
        $this->load->model('ordercart_model','order');      
    ...
    function listorder(){
        ...
        $config['base_url'] 	= base_url().'ordercart/listorder/'.$uri3;
        $config['total_rows']   =  $this->order->get_num_donhangCat($uri3);
        $data['num'] 			= $config['total_rows'];
        $config['per_page']  	= 40;
        $config['uri_segment'] 	= 4; 
        $this->pagination->initialize($config);   
        $data['list'] 			= $this->order->get_all_donhang($config['per_page'],$this->uri->segment(4),$uri3,$field,$order);
        ...
        //*** load templates **********************
        $this->_templates['page'] = 'ordercat/index';
        $this->templates->load($this->_templates['page'],$data);
    }
    /** edit ordercart */
    function edit(){
        $tinhtrangs 	= $this->uri->segment(3);
        $data['title'] 	= 'Xem thông tin đơn đặt hàng';
        $data['save'] 	= true;
        $data['apply'] 	= true;
        $data['cancel'] = 'ordercart/listorder/'.$tinhtrangs;         
            ...
            if($this->vdb->update('shop_cart',$cart,array('order_id'=>$order_id))){
               //write_log(72,233,'Cập nhật đơn hàng: '.$data['info']->barcode); 
                 ...
                 if($sendmail == 1){
                    ...
                    if($option == 'save'){
                       $url = 'ordercart/listorder/'.$tinhtrangs;
                    }else{
                        $url = uri_string();
                    }
                    redirect($url);
        ...
        //load template **********************
        $this->_templates['page'] = 'ordercat/edit';
        $this->templates->load($this->_templates['page'],$data);
    }
    ...
    function view_result(){
        ...
        $this->_templates['page'] = 'ordercat/result';
  ...
  function del(){
      ...
      redirect('ordercart/listorder/'.$page);
  }
  // Xoa nhieu ban ghi
  function dels(){
        ...
        redirect('ordercat/listorder/'.$tinhtrang.'/'.$page);
  }     
}
