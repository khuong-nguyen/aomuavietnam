...
class cpmodules extends CI_Controller{
    ...
    function listmodules(){
        ...
        $config['total_rows']   =  $this->vdb->find_by_num('modules',array('lang'=>vnit_lang(),'site'=>2));
        $data['num'] = $config['total_rows'];
        $config['per_page']  =   20;
        $config['uri_segment'] = 3; 
        $this->pagination->initialize($config);   
        $data['list'] =   $this->vdb->find_by_all('modules',$config['per_page'],$this->uri->segment(4),array('lang'=>vnit_lang(),'site'=>2),$field,$order);
