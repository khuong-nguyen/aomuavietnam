...
class help extends CI_Controller{
    ...
    function __construct(){
        parent::__construct();
        $this->load->model('help_model','tintuc');
        $this->pre_message = "";
    ...
    function baiviet(){
        ...
        $data['add'] = 'help/add|'.icon_add('help/add');
        $data['delete'] = true;     
        ...
        $config['base_url'] 	= base_url().'help/baiviet/';  
    ...
    function add(){
        ...
        if($this->form_validation->run() === FALSE){
        	...
        	if($this->input->post('img')!=''){
                  ...
                  $imgThumb   = 'data/help/thumb/'.$img;
                  vnit_resize_image(ROOT.'data/help/full_images/'.$img,ROOT.$imgThumb,200,200,false);
                  $con['img'] = $img;
                 
            }
            
            if($id = $this->vdb->update('help',$con)){               
    ...
    function edit(){
        ...
        // Form validation
        $data['rs'] = $this->vdb->find_by_id('help',array('id'=>$this->uri->segment(3)));
        $this->form_validation->set_rules('con[name]','Tiêu đề','required');       
        ...
        if($this->form_validation->run() === FALSE){
            ...
            if($this->vdb->update('help',$con,array('id'=>$id))){
    ...
    function dels(){
        ...
        for($i = 0; $i < sizeof($ar_id); $i++){
            if($ar_id[$i]){
                 $title = $this->vdb->find_by_id('help',array('id'=>$ar_id[$i]))->name;
                 $this->vdb->delete('help',array('id'=>$ar_id[$i]));
    ...
    function del(){
        ...
        $title = $this->vdb->find_by_id('help',array('id'=>$newsid))->name;
        if($this->vdb->delete('help',array('id'=>$newsid))){
