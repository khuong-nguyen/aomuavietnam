...
<?php
    ...
    $url 		= 'service/baiviet/0/?option=true'.$cat_url;
...
<?php echo form_open('service/dels',  array('id' => 'admindata'));?> 
<table class="admindata">
    <thead>
        ...
        <tr>
            ...
            <th class="id"><?php echo vnit_order('service/baiviet/0/id/asc','ID')?></th>
            <th><?php echo vnit_order('service/baiviet/0/name/asc','Tiêu đề')?></th>
            <th style="width: 150px;"><?php echo vnit_order('service/baiviet/0/img/asc','Hình ảnh')?></th>
    ...
    $k=1;
    foreach($list as $rs):
    	$img   = (!empty($rs->img))? base_url_site().'data/service/thumb/'.$rs->img: base_url_site().'site/templates/default/images/no_image.gif';
    ?>
    <tr class="row<?php echo $k;?>">
        ...
        <td align="center">
            <?php echo icon_edit('service/edit/'.$rs->id)?>
            <span id="publish<?php echo $rs->id?>"><?php echo icon_active("'service'","'id'",$rs->id,$rs->published)?></span>
            <?php echo icon_del('service/del/'.$rs->id.'/'.(int)$this->uri->segment(4))?>        
        </td>
    </tr>       
    <?php
    $k=1-$k;
