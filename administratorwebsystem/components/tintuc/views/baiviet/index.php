...
<table class="admindata">
    <thead>
        ...
        <tr>
            ...
            <th><?php echo vnit_order('tintuc/baiviet/0/images/asc','Hình ảnh')?></th>
    ...
    $k=1;
    foreach($list as $rs):
    	$img   = (!empty($rs->images))? base_url_site().'data/news/thumb/'.$rs->images: base_url_site().'site/templates/default/images/no_image.gif';
    ?>
    <tr class="row<?=$k?>">
        <td align="center"><input  type="checkbox" name="ar_id[]" value="<?php echo $rs->newsid?>"></td>
        <td><?=$rs->newsid?></td>
        <td align="center"><img src="<?=$img;?>" width="90"></td>
        <td><?=$rs->title?></td>
