...
<table class="admindata">
    ...
    <tr class="row<?=$k?>">
        ...
        <td align="center">
            <?=icon_edit('tintuc/editcomment/'.$rs->commentid.'/'.$this->uri->segment(4))?>
            <span id="publish<?=$rs->commentid?>"><?=icon_active("'shop_comment'","'commentid'",$rs->commentid,$rs->published,'tintuc/publishedcom')?></span>
            <?=icon_del('tintuc/delcomment/'.$rs->commentid.'/'.$this->uri->segment(4))?>        
        </td>
    </tr>       
    <?
    $k=1-$k;
