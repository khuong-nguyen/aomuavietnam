...
function vnit_load_position($position){
    $CI = get_instance();
    $modules['position'] = $position;
    return $CI->load->view('modules/index',$modules);    
}
...
function vnit_lang(){
    $CI = get_instance();
    $lang = $CI->session->userdata('lang_site');
    if($lang == 'vietnamese'){
        return 'vi';
...
function chuyenngay($time){
    $value = date('l, d-m-Y, g:i A',$time);
    $day_en = array('Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday');
    $day_vn = array('Thứ hai','Thứ ba','Thứ tư','Thứ năm','Thứ sáu','Thứ bảy','Chủ nhật');
    return str_replace($day_en,$day_vn,$value);
}
