...
class cart_model extends CI_Model {
    ...
    function get_list_cart() {
        $this->db->select('shop_product.productid, shop_product.producturl,shop_product.productimg, shop_cart_detail.*');
    ...
    function get_list_payment($shipping_id = null) {
        //query
        $this->db->select('payment_id,payment_name,payment_des');
    ...
    function get_shipping_rate($shipping_id) {
        ...
        if ($check->shipping_free == 1) {
            ...
            $mycart = $this->vdb->find_by_id('cheap_cart', array('session_id' => cookie_mycart()));

            $rate = $this->vdb->find_by_id('city', array('city_id' => $mycart->quanhuyen_id));
            return $rate->rate;
        }
    }
    ...
    function save_buoc3() {
        ...
        $data['phivanchuyen'] = $this->get_shipping_rate($this->input->post('shipping_id'));

        $this->db->where('session_id', cookie_mycart());

        if ($this->db->update('cheap_cart', $data)) {
            $rs = $this->vdb->find_by_id('cheap_cart', array('session_id' => cookie_mycart()));
    ...
    function get_listproduct() {
        $this->db->select('shop_product.productid as idProductCart, shop_cart_detail.*');
        $this->db->join('shop_product', 'shop_product.productid = shop_cart_detail.productid');
        $this->db->where('shop_cart_detail.session_id', cookie_mycart());
        return $this->db->get('shop_cart_detail')->result();
    }
    ...
    function get_listproduct_order($cartid) {
        $this->db->select('shop_product.*,shop_product.productid as idProduct, shop_cart_detail.*');
        $this->db->join('shop_product', 'shop_product.productid = shop_cart_detail.productid');
